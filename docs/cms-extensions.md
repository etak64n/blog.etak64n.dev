...
functions/api/admin/
  ...
  link-meta.ts          リンク先ページのタイトルと説明を返す (本文エディタの自動入力用)
functions/favicon/
  [host].ts             リンクカード・参照のファビコンをリンク先から取って同一オリジンで返す (公開)
functions/lib/
  favicon.ts            リンク先サイトのアイコン指定を読んで画像を取る処理
  link-meta.ts          ページの <head> から og:title / description などを読む処理
  fetch-limited.ts      上限バイト数までだけ読む fetch (両方で共用)
templates/partials/
  codebox.html          code と codebox の枠 (色付けとファイル名のタブ)
```
...
- **挿入**: ツールバーの 参照 / 補足 / 注意 / 警告 / コード / リンクカード / 画像、
  または ⌘/ (Windows は Ctrl+/) の一覧から。一覧は「ref」「注意」「code」などで絞り込める。
  codebox は code と同じ枠になるのでボタンは無い (一覧で「コード枠」と探すとコードが出る)
...
- **確定**: 最後の欄で Tab、Esc、または記法の外へキャレットを動かす (クリック・矢印キー) と
  ...
  | 欄 | 例のまま・空のとき |
  |---|---|
  | ref / link の title、link の desc、code の file、img の alt | 引数ごと消す |
  | code の ``` の言語 | 空にする (ファイル名の拡張子から推定される) |
  | URL、ref の引用文、note の本文、コード | 例のまま残す (プレビューで気付ける) |

  一度でも打ち直した欄は、例と同じ文字でも残す。記法の外で打ち始めたときや、欄の外の文字を
  書き換えたときは片付けずに確定する
- **タイトルと説明の自動入力**: リンクカードと参照は、URL を入れて次の欄へ移るか確定すると、
  `/api/admin/link-meta` がリンク先のページを読み、例のままのタイトル (リンクカードは説明も) に
  入れる。先に確定していても、記事の中の同じ記法を探して引数を足す。自分で打った欄は変えない。
  管理 API キー (ヒーロー画像の欄で保存するもの) を使う。取得中と結果は案内行の後ろに出る
...
枠は templates/partials/codebox.html が共通で描き、コードを ``` で囲み直して `markdown` フィルターに
通している。言語は `language` 引数、``` の言語、ファイル名の拡張子の順に決まる。codebox は既存の
記事のために残しているだけで、仕上がりは code と同じ。次の 2 つは色を付けずに表示する。
...
書いた普通の ``` も、base.html のスクリプトが同じタブにする。コピーボタンも、普通の ``` と
code / codebox の枠の両方に付く。プレビューは highlight.js の Nord で色を付けるので、配色は公開ページと
ほぼ同じになる (字句の切り方は Zola と少し違う)。

note の中身は Markdown として描く (公開ページもプレビューも同じ)。中に書いた ref / link / img は
Zola が先に HTML にするので、この 3 つのテンプレートは出力を 1 行に詰めている。字下げや空行を含む
HTML のままだと、Markdown がコードブロックや段落に割ってしまうため。

数式は記事ごとの設定 `extra.math` (管理画面の「数式 (KaTeX)」) で有効にする。区切りは `$$ … $$` と
`\( … \)` で、Markdown が `\` を一つ消すので本文には `\\( … \\)` と書く。`$` 一つの区切りは使わない
(ファミコンの記事の `$0000` のような 16 進表記が数式になってしまうため)。プレビューも同じ区切りで描く。
